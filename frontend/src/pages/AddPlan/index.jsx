...
import { useState } from "react"
 ...
 export function AddPlan() {
    ...
    const save = async (ev) => {
        try {
            ev.preventDefault();
            await axios.post("http://localhost:3001/plan", data);
        ...
        <NavBar/>
          <div className="add-dreambox-form">
            ...
            <div className="add-dreambox-title">
              <span>Adicionar um novo planejamento</span>
            </div>
            ...
            <div className="add-dreambox-text">
              <form onSubmit={save}>
                ...
                <TextInput value={data.name} onChange={
                  ev => setData({...data, name: ev.target.value})}/>

                <label>Objetivo</label>
                <TextInput type={"number"} value={data.goal} onChange={
                  ev => setData({...data, goal: parseFloat(ev.target.value)})}/>
